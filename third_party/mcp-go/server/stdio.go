...
// WithStdioNotificationQueueSize sets the size of the notification queue.
func WithStdioNotificationQueueSize(size int) StdioOption {
	return func(s *StdioServer) {
		const maxQueueSize = 10000
		if size > 0 && size <= maxQueueSize {
			s.notificationQueueSize = size
		} else if size > maxQueueSize {
			s.errLogger.Printf("Notification queue size %d exceeds maximum (%d), using maximum", size, maxQueueSize)
			s.notificationQueueSize = maxQueueSize
		}
	}
}
...
func (s *stdioSession) NotificationQueue() chan mcp.JSONRPCNotification {
	return s.notifications
}
...
var (
	...
	_ SessionWithNotificationQueue = (*stdioSession)(nil)
	_ SessionCloser                = (*stdioSession)(nil)
)
...
func (s *StdioServer) Listen(
	...
	if s.notificationQueueSize > 0 && cap(stdioSessionInstance.notifications) != s.notificationQueueSize {
		stdioSessionInstance.notifications = make(chan mcp.JSONRPCNotification, s.notificationQueueSize)
	}
