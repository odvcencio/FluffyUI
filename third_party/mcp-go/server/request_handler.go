...
func (s *MCPServer) HandleMessage(
	...
	if handleErr != nil {
		if coded, ok := handleErr.(codedError); ok {
	...
	case mcp.MethodResourcesSubscribe:
		...
		if s.capabilities.resources == nil || !s.capabilities.resources.subscribe {
			...
			request.Header = headers
			s.hooks.beforeAny(ctx, baseMessage.ID, mcp.MethodResourcesSubscribe, &request)
		...
		s.hooks.onSuccess(ctx, baseMessage.ID, mcp.MethodResourcesSubscribe, &request, result)
		return createResponse(baseMessage.ID, *result)
	case mcp.MethodResourcesUnsubscribe:
		...
		if s.capabilities.resources == nil || !s.capabilities.resources.subscribe {
			...
			request.Header = headers
			s.hooks.beforeAny(ctx, baseMessage.ID, mcp.MethodResourcesUnsubscribe, &request)
		...
		s.hooks.onSuccess(ctx, baseMessage.ID, mcp.MethodResourcesUnsubscribe, &request, result)
		return createResponse(baseMessage.ID, *result)
