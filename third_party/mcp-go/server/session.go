...
// SessionWithNotificationQueue exposes the underlying notification queue.
// It enables policy-aware queue handling (drop oldest, disconnect, etc).
type SessionWithNotificationQueue interface {
	ClientSession
	NotificationQueue() chan mcp.JSONRPCNotification
}

// SessionCloser allows the server to proactively disconnect a session.
...
func (s *MCPServer) sendNotificationToAllClients(notification mcp.JSONRPCNotification) {
	s.sessions.Range(func(k, v any) bool {
		if session, ok := v.(ClientSession); ok && session.Initialized() {
			_ = s.enqueueNotification(context.Background(), session, notification)
		}
		return true
	})
}
...
func (s *MCPServer) sendNotificationToSpecificClient(session ClientSession, notification mcp.JSONRPCNotification) error {
	return s.enqueueNotification(context.Background(), session, notification)
}
...
func (s *MCPServer) UnregisterSession(
	...
	s.removeAllSubscriptions(sessionID)
...
func (s *MCPServer) sendNotificationCore(
	...
	return s.enqueueNotification(ctx, session, notification)
}
...
func (s *MCPServer) enqueueNotification(ctx context.Context, session ClientSession, notification mcp.JSONRPCNotification) error {
	if session == nil {
		return ErrSessionNotInitialized
	}
	...
	err := s.enqueueWithPolicy(session, notification)
	if err == ErrNotificationChannelBlocked && s.hooks != nil && len(s.hooks.OnError) > 0 {
		method := notification.Method
		hooks := s.hooks
...
func (s *MCPServer) enqueueWithPolicy(session ClientSession, notification mcp.JSONRPCNotification) error {
	policy := strings.ToLower(strings.TrimSpace(s.notificationQueuePolicy))
	switch policy {
	case "", "drop_newest":
		if trySendNotification(session.NotificationChannel(), notification) {
			return nil
		}
		return ErrNotificationChannelBlocked
	case "drop_oldest":
		if queueSession, ok := session.(SessionWithNotificationQueue); ok {
			queue := queueSession.NotificationQueue()
			if trySendQueue(queue, notification) {
				return nil
			}
			if cap(queue) == 0 {
				return ErrNotificationChannelBlocked
...
func (s *MCPServer) disconnectSession(session ClientSession) {
	if session == nil {
		return
	}
	if closer, ok := session.(SessionCloser); ok {
		_ = closer.Close()
	}
	s.UnregisterSession(context.Background(), session.SessionID())
}

func trySendNotification(ch chan<- mcp.JSONRPCNotification, notification mcp.JSONRPCNotification) bool {
	select {
...
func trySendQueue(ch chan mcp.JSONRPCNotification, notification mcp.JSONRPCNotification) bool {
	select {
	case ch <- notification:
		return true
	default:
