...
func requestErrorFrom(err error, id any, fallbackCode int) *requestError {
	if err == nil {
		return nil
	}
...
type MCPServer struct {
	...
	paginationLimit            *int
	notificationQueuePolicy    string
	sessions                   sync.Map
	hooks                      *Hooks
...
// WithNotificationQueuePolicy configures how the server handles full notification queues.
// Accepted values: "drop_oldest", "drop_newest", "disconnect".
func WithNotificationQueuePolicy(policy string) ServerOption {
	return func(s *MCPServer) {
		switch strings.ToLower(strings.TrimSpace(policy)) {
		case "drop_oldest", "drop_newest", "disconnect":
			s.notificationQueuePolicy = policy
		default:
			s.notificationQueuePolicy = ""
		}
	}
}
...
func (s *MCPServer) handleReadResource(
	...
	if ok {
		...
		if err != nil {
			return nil, requestErrorFrom(err, id, mcp.INTERNAL_ERROR)
		}
...
func createErrorResponseWithData(
	...
	return mcp.JSONRPCError{
		JSONRPC: mcp.JSONRPC_VERSION,
		ID:      mcp.NewRequestId(id),
		Error:   mcp.NewJSONRPCErrorDetails(code, message, data),
	}
}
