...
func (s *SegmentedControl) Measure(constraints runtime.Constraints) runtime.Size {
	return s.measureWithStyle(constraints, func(cc runtime.Constraints) runtime.Size {
		width := 2 // opening [ and closing ]
		for i, opt := range s.options {
			width += textWidth(opt) + 2 // space padding around label
...
// Render draws the segmented control.
func (s *SegmentedControl) Render(ctx runtime.RenderContext) {
	...
	// Build the rendered string: [Option A | Option B | Option C]
	...
	// Render character by character, highlighting the selected segment
	x := content.X
	segIdx := -1
	inSegment := false
	for _, r := range line {
		...
		st := baseStyle
		if inSegment && segIdx == s.selected {
			if s.selectedSet || !resolved.IsZero() {
				st = s.selectedStyle
			} else {
				st = s.selectedStyle
			}
...
func (s *SegmentedControl) syncA11y() {
	...
	if s.selected >= 0 && s.selected < len(s.options) {
		s.Base.Value = &accessibility.ValueInfo{
			Text:    s.options[s.selected],
			Current: float64(s.selected + 1),
			Min:     1,
			Max:     float64(len(s.options)),
		}
