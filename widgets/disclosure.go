...
func (d *Disclosure) Measure(constraints runtime.Constraints) runtime.Size {
	return d.measureWithStyle(constraints, func(cc runtime.Constraints) runtime.Size {
		width := textWidth(d.title) + 4 // icon + space + title
		if width < 6 {
			width = 6
		}
		height := 1
		if d.expanded && d.content != nil {
			childC := runtime.Constraints{
				...
				MinHeight: 0,
				MaxHeight: max(0, cc.MaxHeight-1),
			}
...
func (d *Disclosure) Render(ctx runtime.RenderContext) {
	...
	if !resolved.IsZero() {
		final := resolved
		if d.styleSet {
			final = final.Merge(uistyle.FromBackend(d.style))
		}
		if d.focused && d.focusSet {
			final = final.Merge(uistyle.FromBackend(d.focusStyle))
		}
	...
	// Draw header
	icon := "\xe2\x96\xb8" // U+25B8 small right-pointing triangle
	if d.expanded {
		icon = "\xe2\x96\xbe" // U+25BE small down-pointing triangle
	}
...
func (d *Disclosure) ChildWidgets() []runtime.Widget {
	if d == nil || d.content == nil || !d.expanded {
		return nil
	}
